...
namespace Assimalign.Azure.Cosmos.Functions
{
    ...
    /// 
    /// </summary>
    public sealed class CosmosEndsWith : ICosmosFunction
    {
        /// <summary>
        /// 
        /// </summary>
        public CosmosEndsWith() { }
        ...
        public bool TryGetExpression(Expression parameter, out Expression expression)
        {
            ...
            var methodinfo = CosmosUtility.GetEndsWithMethod();
            ...
            {
                expression = CosmosUtility.GetMethodExpression(member, methodinfo, arguments);
            }
            else if (parameter is MethodCallExpression method && method.Method.ReturnType == typeof(string))
            {
                expression = CosmosUtility.GetMethodExpression(method, methodinfo, arguments);
            }
            else
            {
                throw new CosmosInvalidMethodRequestException(
                    message: "The request under '$where' or '$filter' is invalid.", 
                    source: "The method '$endsWith' cannot be used for the requested member or method.");
            }
